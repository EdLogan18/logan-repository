...
CHBase = 'http://pastebin.com/raw/GUci1V8K'
def CHIndex():
	addon_log("CHIndex")
	getData(CHBase,'')
...
addon = xbmcaddon.Addon('plugin.video.listamisterwil')
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(Mr-Wil-Stream,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(Mr-Wil-Stream,New source added.,5000,"+icon+")")
        if not url is None:
