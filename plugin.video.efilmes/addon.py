...
def listar_videos(url):
    ...
    try:
        pagina = soup('div',{"class":"wp-pagenavi"})[0]
        page = soup("a",{"class":"nextpostslink"})
        for prox_pagina in page:
            addDir("[B]"'Próxima Página >>'"[/B]",prox_pagina["href"],2,'http://i.imgur.com/63Qyw7k.png')
    except:
	    pass
...
def pesquisa():
    ...
    if (keyb.isConfirmed()):
        ...
        url = 'http://efilmesnarede.com/?s=' + (parametro_pesquisa)
        listar_videos(url)
...
def player(name,url,iconimage):	
	...
	try:
		...
		nowvideo = r'src=".*?embed.nowvideo.sx/embed.*?v=(.*?)"'
		...
		flashx = r'src=".*?flashx.tv/embed-(.*?)"'
		videott = r'src=".*?videott.*?id=(.*?)"'
		odnoklassniki = r'src=".*?ok.ru/videoembed/(.*?)"'
		youwatch = r'src=".*?youwatch.org/embed-(.*?)"'
		thevid = r'src=".*?thevid.net/e/(.*?)"'
		
		mensagemprogresso = xbmcgui.DialogProgress()
		mensagemprogresso.create('EFILMESNAREDE', 'Procurando fontes ativas para '+name,'Por favor aguarde...')
		...
		try:
			links.append('http://thevid.net/e/'+re.findall(thevid, codigo_fonte)[0])
			hosts.append('Thevid')
		except:
			pass
		...
		try:
			links.append('http://ok.ru/videoembed/'+re.findall(odnoklassniki, codigo_fonte)[0])
			hosts.append('ok.ru')
		except:
			pass		
		...
		try:
			links.append('http://neodrive.co/embed/'+re.findall(neodrive, codigo_fonte)[0])
			hosts.append('Neodrive')
		except:
			pass	
			
		try:
			links.append('http://www.flashx.tv/embed-'+re.findall(flashx, codigo_fonte)[0])
			hosts.append('Flashx')
		except:
			pass			
		...
		elif 'ok.ru' in url_video:
			matriz = obtem_odnoklassniki(url_video)
		elif 'youwatch' in url_video:
			matriz = obtem_youwatch(url_video)
		elif 'thevid.net' in url_video:
			matriz = obtem_thevid(url_video)			
		else:
